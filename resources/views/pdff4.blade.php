...
// Calculate position for the current user



// Calculate position for the current user
$position = calculatePosition($users, $insert);

// Calculate values
$totalScore = calculateTotalScore($insert);
$averageScore = calculateAverageScore($insert);
$grade = calculateGrade($insert);
$remark = calculateRemark($insert);
...
<body>
  ...
  <table>
    <tr>
        ...
        <th><img src="{{ public_path('photos/' . $insert->photo) }}" alt="Report Photo" width="150px" height="150px">
        </th>
  ...
  <table>
    ...
    <table>
        ...
        $subjectScores = [
            ...
            'SocialStudies' => $insert->SocialStudiesScore,
        ...
        foreach ($subjectScores as $subject => $score) {
            // Calculate values for each subject
    ...
    <table>
        <tr>
           <th>Total Marks: {{ $totalScore }}  Average: {{ $averageScore }}  Grade: {{ $grade }} 
                Remark: {{ $remark }}  Position: {{ $position }} Out of: {{ count($users) }} </th>
       </tr>
   </table><br>
